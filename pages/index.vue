  ...
  <v-container>
    <v-dialog v-model="dialog" max-width="355" persistent>
      <v-container class="d-block">
        <v-row no-gutters align="center" justify="space-between">
          ...
          <v-icon @click="dialog=false">md-close</v-icon>
        </v-row>
        <v-form ref="form" v-model="valid">
          <div class="d-flex flex-column">
            ...
            <div class="d-flex flex-column align-center justify-center flex-grow-1 upload-block" @click="chooseImage"
              ...
              <template v-if="!fileToUpload.progress || fileToUpload.progress == 0">
                ...
                <input type="file" accept="jpg, jpeg, png" ref="boardBackground" multiple color="#f66f26"
                  buffer-value="0" @click="onFileClicked($event)" @change="onFileSelected($event)" style="display:none">
              </template>
    ...
    <div class="d-flex flex-wrap align-center justify-start">
      ...
      <v-card
        ...
        @click="$router.push('/boards/'+ board.id)" class="jello-board-tile" v-for="board in boards" :key="board.id">
    ...
    <v-snackbar v-model="snackbar" :timeout="3000" absolute bottom color="primary">
      {{ snackbarText }}
    </v-snackbar>
  </v-container>
</template>
  ...
  export default {
      ...
      methods: {
          ...
          uploadFiles(){
              ...
              return task.on('state_changed', (progress) => {
              ...
              async () => {
                  const url = await task.snapshot.ref.getDownloadURL().catch((e) => false)
